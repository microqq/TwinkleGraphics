...
const vec2 SubSampleOffsets[4] = vec2[](
        vec2(-0.125f, -0.375f),
        vec2( 0.375f, -0.125f),
        vec2(-0.375f,  0.125f),
        vec2( 0.125f,  0.375f)
...
vec2 filterUgly(vec2 v)
{
    return 1.0f - step(0.5f, abs(v));
}

vec2 filterBox(vec2 v)
{
    return filterUgly(v);
}
...
/**
    ...
    0,          |x| >= 1
*/
vec2 filterTriangle(vec2 v)
{
    return clamp(1.0f - abs(v), 0.0f, 1.0f);
}
...
vec2 fitlerCubic(vec2 v)
{
    return vec2(fitlerCubicX(v.x), fitlerCubicX(v.y));
}
...
/**
    ...
    0.42 + 0.5 * cos(2*TW_PI*x/(N-1)) + 0.08 * cos(4*TW_PI*x/(N-1)), |x| < (N-1)/2
    0, otherwise
*/
float blackmanWindow(float x);
vec2 filterBlackman(vec2 v)
{
    return vec2(sinc(v.x), sinc(v.y)) * vec2(blackmanWindow(v.x), blackmanWindow(v.y));
}
...
{
    return vec2(sinc(v.x), sinc(v.y)) * vec2(hanHammingWindow(v.x), hanHammingWindow(v.y));
}
...
float hanHammingWindow(float x)
{
    ...
    if(x < samplecount)
    {
        return(sinc(x) * (0.5f + 0.5f * cos( TW_PI * x / samplecount)));   // a = 0.5f;
    }
...
{
    return vec2(sinc(v.x), sinc(v.y)) * vec2(kaiserWindow(v.x), kaiserWindow(v.y));
}
...
{
    return vec2(sinc(v.x), sinc(v.y)) * vec2(lanczosWindow(v.x, support), lanczosWindow(v.y, support));
}
