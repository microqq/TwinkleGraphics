...
vec4 generateVertex(vec4 p1, vec4 p2, vec4 p, float orientation) {
  ...
  float line_width = thickness * var_line_width / p_cvv.w +
                     (1.0f - var_line_width) * thickness;
  line_width *= 0.5f;
  ...
  } else //直线连接点(joint)
  {
    ...
    **/
    ...
    if (abs(k1 - k2) < epsilon) {
      ...
      //计算直线 (p0, p1)或（p2, p3） 和 (p1, p5) 或（p3,
      //p4）的交点，该交点作为直线连接点（joint） line1:
      ...
      miter.x = (k1 * l1_start.x - k2 * l2_start.x + l2_start.y - l1_start.y) /
                (k1 - k2);
