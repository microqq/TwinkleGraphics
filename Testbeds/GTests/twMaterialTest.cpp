...
TEST(MaterialTests, UniformSetting) {
  ...
  const VecUniform<float32, 4> *cast_vec4 =
      dynamic_cast<const VecUniform<float32, 4> *>(
          material2->GetUniform("tintColor"));
  ASSERT_EQ(cast_vec4->vector, vec4(2.0f, 3.0f, 4.0f, 5.0f));

  ImageData imagedatacopy = GetImageData();
  ASSERT_EQ(imagedatacopy.mip != nullptr &&
                imagedatacopy.mip[0].data != nullptr,
            false);
