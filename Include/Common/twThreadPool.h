...
template <typename T> bool TaskFinished(std::future<T> const &future) {
  ...
  return future.wait_for(std::chrono::microseconds(0ms)) ==
         std::future_status::ready;
}

template <class T> class TSQueue final {
public:
  TSQueue() {}
  ...
  bool EmptyNoLock() { return _queue.empty(); }
...
inline ThreadPool::ThreadPool(uint size) {
  for (uint i = 0; i < size; i++) {
    _workers.emplace_back([this]() -> bool {
      for (;;) {
        Task task;
        {
...
inline ThreadPool::~ThreadPool() { Stop(true); }
...
auto ThreadPool::PushTask(Func &&f, Args &&...args)
  ...
  auto task = std::make_shared<Task>([packedTask]() { (*packedTask)(); });
...
inline void ThreadPool::Stop(bool delay) {
  ...
  for (auto &worker : _workers) {
    if (worker.joinable()) {
      ...
      Console::LogInfo("ThreadPool WorkThread---", worker.get_id(),
                       " stoped.\n");
    }
  }
