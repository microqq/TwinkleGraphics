...
public:
  ...
  void SetMaterial(MaterialPtr material) { _sharedMaterial = material; }

  void SetSharedMaterial(MaterialPtr material) { _sharedMaterial = material; }
  ...
  const MaterialPtr GetSharedMaterial() { return _sharedMaterial; }
  ...
  const MaterialPtr GetMaterial(int32 index) {
    ...
    // _materials[index] =
    // std::make_shared<Material>(*(_sharedMaterials[index].get()));
...
private:
  void BindingVertexBuffer(int32 attribIndex, int32 attribSize,
                           int32 bindingIndex, int32 offset, int32 stride);
