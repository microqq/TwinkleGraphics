...
public:
  ...
  MeshDataFlag GetMeshDataFlag() {
    if (_mesh != nullptr)
      return _mesh->GetMeshDataFlag();
    else
      return MeshDataFlag::DEFAULT;
  }
...
public:
  ...
  Triangle(glm::vec3 p0, glm::vec3 p1, glm::vec3 p2,
      ...
      : Geometry(), _p0(p0), _p1(p1), _p2(p2) {
    GenerateMeshInternal(flag);
  }
  virtual ~Triangle() {}

  virtual void
  GenerateMeshInternal(MeshDataFlag flag = MeshDataFlag::DEFAULT) override {
    if (_mesh == nullptr) {
...
public:
  ...
  UVSphere(float32 radius = 1.0f, int32 subdivision = 20,
           MeshDataFlag flag = MeshDataFlag::DEFAULT)
      : Geometry(), _radius(radius), _subdivision(subdivision) {
    GenerateMeshInternal(flag);
  }
  virtual ~UVSphere() {}
  ...
  GenerateMeshInternal(MeshDataFlag flag = MeshDataFlag::DEFAULT) override {
    if (_mesh == nullptr) {
      _mesh =
          CreateSphereMeshStandard(_radius, _subdivision, _subdivision, flag);
    }
  }
...
private:
   ...
   */
  MeshPtr CreateSphereMeshStandard(float32 radius, int32 longitude_count,
...
private:
   ...
   */
  MeshPtr
  CreateSphereMeshIcosahedron(float32 radius, int32 subdivide,
                              MeshDataFlag flag = MeshDataFlag::DEFAULT) {
    ...
    // CreateIconsahedron(submesh->GetVerticePos(), submesh->GetIndice(),
    // radius);
      ...
      for (int32 row = 0, row_count = subdivide + 1; row < row_count; row++) {
        ...
        for (; col < colCount; col++) {
          if (row == 0) {
            ...
            p = p_helper1 +
                (p_helper2 - p_helper1) * (float32)col / (float32)row;
          }
          p = glm::normalize(p);
          p *= radius;
          ...
          if (row != 0) {
            if (col != 0) {
              ...
              submesh->_indice[indice_index + 1] =
...
private:
  MeshPtr CreateCubeMesh(float32 size,
                         MeshDataFlag flag = MeshDataFlag::DEFAULT) {
    MeshPtr mesh = std::make_shared<Mesh>();
...
protected:
  MeshPtr CreateQuadMesh(float32 x_size, float32 y_size,
                         MeshDataFlag flag = MeshDataFlag::DEFAULT) {
    MeshPtr mesh = std::make_shared<Mesh>();
...
public:
  ...
  Plane(glm::vec3 normal, float32 width, int32 subdivision = 32,
        MeshDataFlag flag = MeshDataFlag::DEFAULT)
      : Geometry(), _normal(normal), _width(width), _subdivision(subdivision) {
    GenerateMeshInternal(flag);
  }
  virtual ~Plane() {}
...
protected:
  MeshPtr CreatePlaneMesh(MeshDataFlag flag = MeshDataFlag::DEFAULT) {
    ...
    glm::vec4 *uvs = mesh->GetVerticeUV();
    glm::vec3 axis =
        glm::normalize(glm::cross(glm::vec3(0.0f, 0.0f, 1.0f), _normal));
...
public:
  ...
  BezierCurve(glm::vec4 *points, int32 numPoints, int32 degree,
              int32 segments = 128)
      : Geometry(), _controlPoints(nullptr), _pointsCount(degree + 1) {}

  virtual ~BezierCurve() { SAFE_DEL_ARR(_controlPoints); }
...
public:
  ...
  BSplineCurve(int32 n, int32 p, int32 numKnots, Knot *knots,
      ...
      : Geometry(), _controlPoints(nullptr), _knots(nullptr), _pointsCount(n),
        _knotsCount(n + p + 1), _degree(p) {
  ...
  GenerateMeshInternal(MeshDataFlag flag = MeshDataFlag::DEFAULT) override {
    if (_mesh == nullptr) {
      _mesh = Mesh::CreateLineMesh(
          nullptr, (_knotsCount - 1 - 2 * _degree) * _segments + 1);
    }
  ...
  glm::vec3 DeBoor(float32 u, int32 k) {
    ...
    for (int32 r = 1; r <= h; r++) {
      for (int32 j = k - _degree + r, n = k - s, g = 1; j <= n; j++) {
        float32 a =
            (u - _knots[j].u) / (_knots[j + _degree - r + 1].u - _knots[j].u);
        if (r == 1) {
          helper[g - 1] =
              (1.0f - a) * _controlPoints[j - 1] + a * _controlPoints[j];
        } else {
    ...
    glm::vec3 result =
        glm::vec3(helper[0].x, helper[0].y, helper[0].z) / helper[0].w;
...
public:
  ...
  BezierSurface(int32 n1, int32 n2)
      : Geometry(), _control_points(nullptr), _v_knots(nullptr),
        _u_knots(nullptr), _u_points_count(n1), _v_points_count(n2),
        _u_degree(n1 - 1), _v_degree(n2 - 1) {}
...
public:
  ...
  NURBSSurface(int32 n1, int32 p1, int32 n2, int32 p2,
               ...
               Knot *vKnots = nullptr, bool genMesh = true,
               int32 subdivide = 16, bool rational = false,
               MeshDataFlag flag = MeshDataFlag(13))
      : Geometry(), _controlPoints(nullptr), _vKnots(nullptr), _uKnots(nullptr),
        _apduSurface(nullptr), _apdvSurface(nullptr), _uPointsCount(n1),
        _vPointsCount(n2), _uKnotsCount(n1 + p1 + 1), _vKnotsCount(n2 + p2 + 1),
        _uDegree(p1), _vDegree(p2), _subdivide(subdivide), _rational(rational) {
...
private:
  void InitializeAPDUSurface() {
    if (_apduSurface == nullptr) {
      _apduSurface = std::make_shared<NURBSSurface>(
          _uPointsCount - 1, _uDegree - 1, _vPointsCount, _vDegree, nullptr,
      ...
      if (_uKnots[_uKnotsCount - 1].multiplity > 1) {
        uknots[_uKnotsCount - 3].multiplity =
            _uKnots[_uKnotsCount - 1].multiplity;
      }
  ...
  void InitializeAPDVSurface() {
    if (_apdvSurface == nullptr) {
      _apdvSurface = std::make_shared<NURBSSurface>(
          _uPointsCount, _uDegree, _vPointsCount - 1, _vDegree - 1, nullptr,
          nullptr, nullptr, false);
      ...
      if (_vKnots[_vKnotsCount - 1].multiplity > 1) {
        vknots[_vKnotsCount - 3].multiplity =
            _vKnots[_vKnotsCount - 1].multiplity;
      }
  ...
  glm::vec4 DeBoor(float32 u, int32 k, int32 p, Knot *knots,
                   glm::vec4 *points) {
    int32 s = 0;
    if (u == knots[k].u) {
  ...
  glm::vec3 ComputeNormal(float32 u, float32 v, int32 uspan, int32 vspan,
                          glm::vec3 suv) {
    float32 w =
