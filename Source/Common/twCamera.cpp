...
Camera::Camera(Viewport viewport, float32 fov, float32 n, float32 f,
               Camera::Type type)
    : SceneNode(), _viewport(viewport), _rendertarget(nullptr),
      _frustum(nullptr), _fov(fov), _aspect(viewport.AspectRatio()), _near(n),
      _far(f), _type(type), _cullingmask((int32)CullingMask::EVERYTHING),
      _sortdepth(-1), _viewportDirty(true) {
  SetupProjection();
}

Camera::Camera(Viewport viewport, float32 n, float32 f, Camera::Type type)
    : SceneNode(), _viewport(viewport), _rendertarget(nullptr), _near(n),
      _far(f), _type(type), _cullingmask((int32)CullingMask::EVERYTHING),
      _sortdepth(-1), _viewportDirty(true) {}
