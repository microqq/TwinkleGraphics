...
ImageManager::ImageManager() : IUpdatable(), INonCopyable(), IDestroyable() {
...
ImagePtr ImageManager::ReadImage(const char *filename, ImageOption *option) {
  ...
  ReadResult<Image> result =
      resMgr.Read<ImageReader, Image, ImageOption>(filename, option);
...
template <>
void ResourceManager::PackedReadTask<ReadResult<Image>,
                                     ImageReader>::PushTask() {
  // typedef Ret(R::*)(const char*, ReaderOption) Func;
  ResourceManager &resMgr = ResourceMgrInstance();
  if (_asyncRead) {
    auto future =
        resMgr.PushAsyncTask(&ImageReader::ReadAsync, _reader, _filename);
    {
