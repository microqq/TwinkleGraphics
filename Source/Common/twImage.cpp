...
Image::Image() : Object() { _source = std::make_shared<ImageSource>(); }

Image::Image(const char *filename, const ImageData &data) : Object() {
...
ImageReader::ImageReader() : ResourceReader() {
  // INITIALISE_READERID
}

ImageReader::ImageReader(ImageOption *option) : ResourceReader() {
  if (option != nullptr) {
    if (_option != nullptr) {
      SAFE_DEL(_option);
    }
...
ReadResult<Image> ImageReader::ReadDDS(const char *filename) {
  ...
  if (vglLoadDDS(filename, &data)) {
    ...
    return ReadResult<Image>(shared_from_this(), image,
                             ReadResult<Image>::Status::SUCCESS);
  }
...
ReadResult<Image> ImageReader::ReadOthers(const char *filename) {
  ...
  // return success
  return ReadResult<Image>(shared_from_this(), ret_image,
                           ReadResult<Image>::Status::SUCCESS);
}
