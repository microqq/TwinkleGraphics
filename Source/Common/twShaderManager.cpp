...
ShaderManager &ShaderMgrInstance() {
  return Singleton<ShaderManager>::Instance();
}

ShaderManager::ShaderManager()
    : IUpdatable(), INonCopyable(), IDestroyable(), _futures(),
      _programFutures(), _mutex(), _programMutex() {}
...
ShaderPtr ShaderManager::ReadShader(const char *filename,
                                    ShaderOption *option) {
  ...
  if (!macros.empty()) {
    shaderFilename +=
        std::string(":") + std::to_string(std::hash<std::string>{}(macros));
  }

  ReadResult<Shader> result = resMgr.Read<ShaderReader, Shader, ShaderOption>(
      shaderFilename.c_str(), option);
...
ShaderProgramPtr ShaderManager::ReadShaders(ShaderOption options[], int32 num) {
  ...
  for (int i = 0; i < num; i++) {
    ShaderPtr shader =
        ReadShader(options[i]._optionData.filename.c_str(), &options[i]);
    if (shader != nullptr) {
...
ReadResult<Shader> ShaderManager::ReadShaderAsync(const char *filename,
                                                  ShaderOption *option) {
  ResourceManager &resMgr = ResourceMgrInstance();
  ...
  return resMgr.ReadAsync<ShaderReader, Shader, ShaderOption>(
      shaderFilename.c_str(), option);
}

ReadResult<ShaderProgram>
ShaderManager::ReadShadersAsync(ShaderProgramOption *option, int32 num) {
  ResourceManager &resMgr = ResourceMgrInstance();
  ...
  if (!option->_macros.empty()) {
    programFilename +=
        std::string(":") +
        std::to_string(std::hash<std::string>{}(option->_macros));
  }

  return resMgr.ReadAsync<ShaderReader, ShaderProgram, ShaderProgramOption>(
      programFilename.c_str(), option);
}
...
void ShaderManager::AddTaskProgramFuture(
    std::future<ReadResult<ShaderProgram>> future) {
  {
    std::lock_guard<std::mutex> lock(_programMutex);
...
template <>
void ResourceManager::PackedReadTask<ReadResult<Shader>,
                                     ShaderReader>::PushTask() {
  // typedef Ret(R::*)(const char*, ReaderOption) Func;
  ResourceManager &resMgr = ResourceMgrInstance();
  if (_asyncRead) {
    auto future =
        resMgr.PushAsyncTask(&ShaderReader::ReadAsync, _reader, _filename);
    {
...
template <>
void ResourceManager::PackedReadTask<ReadResult<ShaderProgram>,
                                     ShaderReader>::PushTask() {
  // typedef Ret(R::*)(const char*, ReaderOption) Func;
  ResourceManager &resMgr = ResourceMgrInstance();
  if (_asyncRead) {
    auto future = resMgr.PushAsyncTask(&ShaderReader::ReadProgramAsync, _reader,
                                       _filename);
    {
      ShaderManager &shaderMgr = ShaderMgrInstance();
