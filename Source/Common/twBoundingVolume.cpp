...
vec3 AABoundingBox::GetCorner(int index) const {
  ...
  return vec3((index & 1) ? _max.x : _min.x, (index & 2) ? _max.y : _min.y,
...
bool AABoundingBox::ContainPoint(const vec3 &point) const {
  return _max.x >= point.x && _min.x <= point.x && _max.y >= point.y &&
         _min.y <= point.y && _max.z >= point.z && _min.z <= point.z;
}
...
bool AABoundingBox::IsEmpty() const { return _max == _min; }
...
bool AABoundingBox::Intersect(const AABoundingBox &other) const {
  ...
  return _max.x >= other._min.x && _max.y >= other._min.y &&
         _max.z >= other._min.z && _min.x <= other._max.x &&
         _min.y <= other._max.y && _min.z <= other._max.z;
}
 ...
 *
 */
bool AABoundingBox::Intersect(const vec3 &origin, const vec3 &dir, float &t,
...
/**
 * @brief
 *  <<Real-Time Rendering 4th Rendering>> Chapter22: Plane and box intersection,
 Page 971.
 *  (1) "One way to determine whether a box intersects a plane is to insert all
 the vertices of the box into the plane equation" (2) "The idea behind both
 methods is that only two of the eight corners need to be inserted into the
 plane equation" (3) "Every box has four diagonals, formed by its corners.
 Taking the dot product of each diagonal’s direction with the plane’s normal,
 the largest value identifies the diagonal with these two furthest points. By
 testing just these two corners, the box as a whole is tested against a plane"
    (4) "Why is this equivalent to finding the maximum of the eight different
 half diagonals projections?"
 * @param planeNormal
 * @param distance
 * @return true
 * @return false
 */
bool AABoundingBox::Intersect(const vec3 &planeNormal, float d,
                              Intersection &intersection) const {
  // Plane Equation: Dot((A, B, C), (x, y, z)) + D = 0;
  // Compute the maximum of the eight different half diagnoals projections.
  // How? These eight half diagonals are the combinations: gi = (±hx, ±hy, ±hz),
  // and we want to compute gi · n for all eight i.
  ...
  // Box center to plane Distance Equation: distance = -Dot(Center, Normal) + D)
  // / Dot(Normal, Normal). if Normal normalized, Dot(Normal, Normal) == 1. So,
  // distance = -Dot(Center, Normal) - D. optimise: we use distance = -distance
  float s = glm::dot((_max + _min) * 0.5f, planeNormal) + d;

  // <<Real-Time Rendering 4th Rendering>> Chapter22: Figure 22.18, "Assuming
  // that the “outside” of the plane is the positive half-space"
  intersection = (s - e > 0.0f)   ? OUTSIDE
                 : (s + e < 0.0f) ? INSIDE
                                  : INTERSECTING;
...
bool AABoundingBox::IntersectLineSegment(const vec3 &origin, const vec3 &dir,
...
OrientedBoundingBox::OrientedBoundingBox(const vec3 &center,
                                         const std::array<vec3, 3> &axis,
                                         const vec3 &extents)
    : Object(), _axis(axis), _center(center), _extents(extents) {}

OrientedBoundingBox::OrientedBoundingBox(const OrientedBoundingBox &other)
    : Object(other), _axis(other._axis), _center(other._center),
...
bool OrientedBoundingBox::Intersect(const vec3 &planeNormal, float d,
                                    Intersection &intersection) const {
  return false;
}

bool OrientedBoundingBox::Intersect(const vec3 &origin, const vec3 &dir,
                                    float tMin, float tMax) const {
  return false;
}
...
bool BoundingSphere::Intersect(const vec3 &origin, const vec3 &dir, float tMin,
                               float tMax) const {
  float const infinity = std::numeric_limits<float>::max();
  ...
  vec3 intersectPoint;
  float squareDist =
      SquareDistancePoint2Line(_center, origin, dir, intersectPoint);
  float squareR = _radius * _radius;
  if (squareDist > squareR)
    return false;

  if (tMax == infinity) {
    Console::LogAssert(tMin == 0.0f || tMin == -infinity,
                       "Ray(or Line) tMin has a value error.", "\n");
  ...
  // if segment, tMin != (-infinity || inifinity), tMax != (-infinity ||
  // inifinity)
  Console::LogAssert(tMin != infinity && tMax != infinity && tMin >= 0.0f &&
                         tMax > tMin,
                     "LineSegment tMin/tMax has a value error.", "\n");
  return true;
}
} // namespace TwinkleGraphics
