...
Texture::Texture(bool immutable, bool genMipMap)
    : Object(), _res(), _image(nullptr), _sampler(nullptr),
      _mask(TexParameterMask::TEXPARAMETER_DEFAULT_MASK),
      _dirtyFlag(TexParameterMask::TEXPARAMETER_DEFAULT_DIRTY_FLAG),
      _internalformat(GL_NONE), _format(GL_NONE), _miplevels(0), _width(-1),
      _height(-1), _depth(-1), _immutable(immutable),
...
template <>
void __TWAPI Texture::SetWrap<WrapParam::WRAP_S>(WrapMode wrap) {
  if (wrap == _parameters.wrapModes[0])
    return;
  _parameters.wrapModes[0] =
      (wrap == WrapMode::NONE ? WrapMode::DEFAULT : wrap);
  _dirtyFlag =
      (TexParameterDirtyFlag)(_dirtyFlag |
                              TexParameterDirtyFlag::TEXPARAMETER_WRAP_S_MASK);
}

template <>
void __TWAPI Texture::SetWrap<WrapParam::WRAP_T>(WrapMode wrap) {
  if (wrap == _parameters.wrapModes[1])
    return;
  _parameters.wrapModes[1] =
      (wrap == WrapMode::NONE ? WrapMode::DEFAULT : wrap);
  _dirtyFlag =
      (TexParameterDirtyFlag)(_dirtyFlag |
                              TexParameterDirtyFlag::TEXPARAMETER_WRAP_T_MASK);
}

template <>
void __TWAPI Texture::SetWrap<WrapParam::WRAP_R>(WrapMode wrap) {
  if (wrap == _parameters.wrapModes[2])
    return;

  _parameters.wrapModes[2] =
      (wrap == WrapMode::NONE ? WrapMode::DEFAULT : wrap);
  _dirtyFlag =
      (TexParameterDirtyFlag)(_dirtyFlag |
                              TexParameterDirtyFlag::TEXPARAMETER_WRAP_R_MASK);
}

template <>
void __TWAPI
Texture::SetFilter<FilterParam::MIN_FILTER>(FilterMode filter) {
  if (filter == _parameters.filterModes[0])
    return;
  _parameters.filterModes[0] =
      (filter == FilterMode::NONE ? FilterMode::DEFAULT : filter);
  _dirtyFlag =
      (TexParameterDirtyFlag)(_dirtyFlag | TexParameterDirtyFlag::
                                               TEXPARAMETER_FILTER_MIN_MASK);
}

template <>
void __TWAPI
Texture::SetFilter<FilterParam::MAG_FILTER>(FilterMode filter) {
  if (filter == _parameters.filterModes[1])
    return;
  _parameters.filterModes[1] =
      (filter == FilterMode::NONE ? FilterMode::DEFAULT : filter);
  _dirtyFlag =
      (TexParameterDirtyFlag)(_dirtyFlag | TexParameterDirtyFlag::
...
template <>
void __TWAPI Texture::SetSwizzle<1>(SwizzleParam parameter,
                                          Swizzle1 swizzle) {
...
template <>
void __TWAPI Texture::SetSwizzle<4>(SwizzleParam parameter,
                                          Swizzle4 swizzle) {
  _parameters.swizzleParameter = parameter;
...
void Texture::SetSwizzle(SwizzleParam parameter, SwizzleMask *masks) {
  ...
  int32 val =
      (uint32)_parameters.swizzleParameter - (uint32)SwizzleParam::SWIZZLE_R;
  if (val >= 0 && val < 4) {
...
void Texture::SetLodBias(LodBiasParam parameter, float32 bias) {
  ...
  _parameters.lodParameter =
      parameter == LodBiasParam::NONE ? LodBiasParam::DEFAULT : parameter;
  _parameters.lodbias = parameter == LodBiasParam::NONE ? 0.0f : bias;

  if (dirty) {
    _dirtyFlag =
        (TexParameterDirtyFlag)(_dirtyFlag | TexParameterDirtyFlag::
                                                 TEXPARAMETER_LODBIAS_MASK);
  }
}
...
void Texture::SetMipMapBaseLevel(MipMapBaseLevelParam parameter, int32 level) {
  ...
  if (dirty) {
    _dirtyFlag =
        (TexParameterDirtyFlag)(_dirtyFlag |
                                TexParameterDirtyFlag::
                                    TEXPARAMETER_MIPMAP_BASE_LEVEL_MASK);
  }
}
...
void Texture::SetTexBorderColor(TextureBorderColorParam parameter,
                                const vec4 &color) {
...
void Texture::SetDepthTexCompareMode(DepthTextureCompareMode mode) {
  ...
  _dirtyFlag =
      (TexParameterDirtyFlag)(_dirtyFlag |
                              TexParameterDirtyFlag::
                                  TEXPARAMETER_DEPTH_TEX_COMP_MODE_MASK);
}
...
void Texture::SetDepthTexCompareFun(DepthTextureCompareFunc func) {
  ...
  _dirtyFlag =
      (TexParameterDirtyFlag)(_dirtyFlag |
                              TexParameterDirtyFlag::
                                  TEXPARAMETER_DEPTH_TEX_COMP_FUNC_MASK);
}
...
void Texture::SetDepthStencilMode(DepthStencilTextureMode mode) {
  ...
  _dirtyFlag =
      (TexParameterDirtyFlag)(_dirtyFlag | TexParameterDirtyFlag::
                                               TEXPARAMETER_DEPTHSTENCIL_MASK);
}
...
void Texture::SetAnistropic(float32 anistropic, bool maxAnistropic) {
  ...
  _dirtyFlag = (TexParameterDirtyFlag)(_dirtyFlag |
                                       TexParameterDirtyFlag::
                                           TEXPARAMETER_ANISTROPIC_FILTER_MASK);
}
...
void Texture::ApplyWrapParameter() {
  ...
  if (wrap_s_mask &&
    ...
    _dirtyFlag = (TexParameterDirtyFlag)((uint32)_dirtyFlag &
                                         (!((uint32)TexParameterDirtyFlag::
                                                TEXPARAMETER_WRAP_S_MASK)));
  }
  if (wrap_t_mask &&
  ...
  if (wrap_r_mask &&
    ...
    if (useSampler)
      glSamplerParameteri(samplerId, GL_TEXTURE_WRAP_S,
                          (int32)(_parameters.wrapModes[2]));
    else
      glTexParameteri(_res.type, GL_TEXTURE_WRAP_R,
                      (int32)(_parameters.wrapModes[2]));

    _dirtyFlag = (TexParameterDirtyFlag)((uint32)_dirtyFlag &
                                         (!((uint32)TexParameterDirtyFlag::
                                                TEXPARAMETER_WRAP_R_MASK)));
  }
}

void Texture::ApplyFilterParameter() {
  bool filter_min_mask =
      (_mask & TexParameterMask::TEXPARAMETER_FILTER_MIN_MASK) != 0;
  bool filter_mag_mask =
      (_mask & TexParameterMask::TEXPARAMETER_FILTER_MAG_MASK) != 0;
  ...
  // Minification filter
  if (filter_min_mask &&
      (_dirtyFlag & TexParameterDirtyFlag::TEXPARAMETER_FILTER_MIN_MASK) != 0) {
    if (useSampler)
      glSamplerParameteri(samplerId, GL_TEXTURE_MIN_FILTER,
                          (int32)(_parameters.filterModes[0]));
    else
      glTexParameteri(_res.type, GL_TEXTURE_MIN_FILTER,
                      (int32)(_parameters.filterModes[0]));
  ...
  if (filter_mag_mask &&
      (_dirtyFlag & TexParameterDirtyFlag::TEXPARAMETER_FILTER_MAG_MASK) != 0) {
    if (useSampler)
      glSamplerParameteri(samplerId, GL_TEXTURE_MAG_FILTER,
                          (int32)(_parameters.filterModes[1]));
    else
      glTexParameteri(_res.type, GL_TEXTURE_MAG_FILTER,
                      (int32)(_parameters.filterModes[1]));
...
void Texture::ApplySwizzleParameter() {
  bool swizzle_mask =
      (_mask & TexParameterMask::TEXPARAMETER_SWIZZLE_MASK) != 0;

  // swizzle
  if (swizzle_mask &&
      (_dirtyFlag & TexParameterDirtyFlag::TEXPARAMETER_SWIZZLE_MASK) != 0) {
    uint32 swizzle_param = uint32(_parameters.swizzleParameter);
    int32 index = swizzle_param - (uint32)(SwizzleParam::SWIZZLE_R);
    if (index == 4) {
      glTexParameteriv(_res.type, swizzle_param,
                       reinterpret_cast<const int32 *>(_parameters.swizzle));
    } else {
      glTexParameteri(_res.type, swizzle_param,
                      (int32)(_parameters.swizzle[index]));
    }
...
void Texture::ApplyLodBias() {
  bool lodbias_mask =
      (_mask & TexParameterMask::TEXPARAMETER_LODBIAS_MASK) != 0;
  ...
  // lod bias
  if (lodbias_mask && _dirtyFlag &&
      (TexParameterDirtyFlag::TEXPARAMETER_LODBIAS_MASK != 0)) {
    if (useSampler)
...
void Texture::ApplyDepthTexCompParameter() {
  bool depth_tex_mode_mask =
      (_mask & TexParameterMask::TEXPARAMETER_DEPTH_TEX_COMP_MODE_MASK) != 0;
  bool depth_tex_func_mask =
      (_mask & TexParameterMask::TEXPARAMETER_DEPTH_TEX_COMP_FUNC_MASK) != 0;
  ...
  if (depth_tex_mode_mask &&
      (_dirtyFlag &
       TexParameterDirtyFlag::TEXPARAMETER_DEPTH_TEX_COMP_MODE_MASK) != 0) {
    _dirtyFlag =
        (TexParameterDirtyFlag)((uint32)_dirtyFlag &
                                (!((uint32)TexParameterDirtyFlag::
                                       TEXPARAMETER_DEPTH_TEX_COMP_MODE_MASK)));
  }

  // depth tex compare func
  if (depth_tex_func_mask &&
      (_dirtyFlag &
       TexParameterDirtyFlag::TEXPARAMETER_DEPTH_TEX_COMP_FUNC_MASK) != 0) {
    _dirtyFlag =
        (TexParameterDirtyFlag)((uint32)_dirtyFlag &
                                (!((uint32)TexParameterDirtyFlag::
                                       TEXPARAMETER_DEPTH_TEX_COMP_FUNC_MASK)));
...
void Texture::ApplyMipMapParameter() {
  bool mipmap_base_level_mask =
      (_mask & TexParameterMask::TEXPARAMETER_MIPMAP_BASE_LEVEL_MASK) != 0;
  bool mipmap_max_level_mask =
      (_mask & TexParameterMask::TEXPARAMETER_MIPMAP_MAX_LEVEL_MASK) != 0;
  ...
  if (mipmap_base_level_mask &&
      (_dirtyFlag &
       TexParameterDirtyFlag::TEXPARAMETER_MIPMAP_BASE_LEVEL_MASK) != 0) {
    glTexParameterf(_res.type, GL_TEXTURE_BASE_LEVEL,
                    _parameters.mipmapBaselevel);
  ...
  if (mipmap_max_level_mask &&
      (_dirtyFlag &
       TexParameterDirtyFlag::TEXPARAMETER_MIPMAP_MAX_LEVEL_MASK) != 0) {
    glTexParameterf(_res.type, GL_TEXTURE_MAX_LEVEL,
                    _parameters.mipmapMaxlevel);

    _dirtyFlag =
        (TexParameterDirtyFlag)((uint32)_dirtyFlag &
                                (!((uint32)TexParameterDirtyFlag::
                                       TEXPARAMETER_MIPMAP_MAX_LEVEL_MASK)));
  }
}

void Texture::ApplyTexLODParameter() {
  bool tex_min_lod_mask =
      (_mask & TexParameterMask::TEXPARAMETER_MIN_LOD_MASK) != 0;
  bool tex_max_lod_mask =
      (_mask & TexParameterMask::TEXPARAMETER_MAX_LOD_MASK) != 0;
  ...
  if (tex_min_lod_mask &&
      (_dirtyFlag & TexParameterDirtyFlag::TEXPARAMETER_MIN_LOD_MASK) != 0) {
    _dirtyFlag = (TexParameterDirtyFlag)((uint32)_dirtyFlag &
                                         (!((uint32)TexParameterDirtyFlag::
                                                TEXPARAMETER_MIN_LOD_MASK)));
  }
  ...
  if (tex_max_lod_mask &&
      (_dirtyFlag & TexParameterDirtyFlag::TEXPARAMETER_MAX_LOD_MASK) != 0) {
    _dirtyFlag = (TexParameterDirtyFlag)((uint32)_dirtyFlag &
                                         (!((uint32)TexParameterDirtyFlag::
                                                TEXPARAMETER_MAX_LOD_MASK)));
  }
...
void Texture::ApplyTexBorderColor() {
  bool border_color_mask =
      (_mask & TexParameterMask::TEXPARAMETER_BORDERCOLOR_MASK) != 0;
  ...
  if (border_color_mask &&
      (_dirtyFlag & TexParameterDirtyFlag::TEXPARAMETER_BORDERCOLOR_MASK) !=
          0) {
    if (useSampler)
...
void Texture::ApplyDepthStencilMode() {
  bool depthstencil_mask =
      (_mask & TexParameterMask::TEXPARAMETER_DEPTHSTENCIL_MASK) != 0;
  ...
  if (depthstencil_mask &&
      (_dirtyFlag & TexParameterDirtyFlag::TEXPARAMETER_DEPTHSTENCIL_MASK) !=
          0) {
...
void Texture::ApplyAnistropicFilter() {
  bool mask =
      (_mask & TexParameterMask::TEXPARAMETER_ANISTROPIC_FILTER_MASK) != 0;

  if (mask &&
      (_dirtyFlag &
       TexParameterDirtyFlag::TEXPARAMETER_ANISTROPIC_FILTER_MASK) != 0) {
    float32 anistropic;
...
void Texture1D::InitStorage() {
  ...
  if (srcimage != nullptr) {
    if (_immutable) {
      glTexStorage1D(_res.type, srcimage->mipLevels, srcimage->internalFormat,
                     srcimage->mip[0].width);

      for (int32 level = 0, mipLevels = srcimage->mipLevels; level < mipLevels;
           ++level) {
        glTexSubImage1D(_res.type, level, 0, srcimage->mip[level].width,
      ...
      for (int32 level = 0, mipLevels = srcimage->mipLevels; level < mipLevels;
           ++level) {
        glTexImage1D(_res.type, level, srcimage->internalFormat,
                     srcimage->mip[level].width, 0, srcimage->format,
                     srcimage->type, srcimage->mip[level].data);
      }
    }
    ...
    glTexStorage1D(_res.type, _miplevels, _internalformat, _width);
  }
...
void Texture2D::InitStorage() {
  ...
  if (srcimage != nullptr) {
    if (_immutable) {
      glTexStorage2D(_res.type, srcimage->mipLevels, srcimage->internalFormat,
                     srcimage->mip[0].width, srcimage->mip[0].height);

      for (int32 level = 0, mipLevels = srcimage->mipLevels; level < mipLevels;
           ++level) {
        glTexSubImage2D(_res.type, level, 0, 0, srcimage->mip[level].width,
                        srcimage->mip[level].height, srcimage->format,
                        srcimage->type, srcimage->mip[level].data);
      }
    } else {
      for (int32 level = 0, mipLevels = srcimage->mipLevels; level < mipLevels;
           ++level) {
        glTexImage2D(_res.type, level, srcimage->internalFormat,
                     srcimage->mip[level].width, srcimage->mip[level].height, 0,
                     srcimage->format, srcimage->type,
                     srcimage->mip[level].data);
      }
    }
    ...
    if (_immutable) {
      glTexStorage2D(_res.type, _miplevels, _internalformat, _width, _height);
    } else {
      glTexImage2D(_res.type, 0, _internalformat, _width, _height, 0, _format,
...
void Texture2D::ResizeStorage(int32 width, int32 height) {
  if (!_immutable) {
    glBindTexture(_res.type, _res.id);
    _width = width;
    _height = height;
...
void Texture2DMultiSample::InitStorage() {
  ...
  if (_immutable) {
    glTexStorage2DMultisample(_res.type, _samples, _internalformat, _width,
                              _height, _fixedsampledlocation);
  } else {
    glTexImage2DMultisample(_res.type, _samples, _internalformat, _width,
                            _height, _fixedsampledlocation);
  }
...
void Texture2DMultiSample::ResizeStorage(int32 width, int32 height) {
  if (!_immutable) {
    glBindTexture(_res.type, _res.id);
    _width = width;
    _height = height;
...
void Texture3D::InitStorage() {
  ...
  if (srcimage != nullptr) {
    if (_immutable) {
      glTexStorage3D(_res.type, srcimage->mipLevels, srcimage->internalFormat,
                     srcimage->mip[0].width, srcimage->mip[0].height,
                     srcimage->mip[0].depth);

      for (int32 level = 0, mipLevels = srcimage->mipLevels; level < mipLevels;
           ++level) {
        glTexSubImage3D(_res.type, level, 0, 0, 0, srcimage->mip[level].width,
                        srcimage->mip[level].height, srcimage->mip[level].depth,
                        srcimage->format, srcimage->type,
                        srcimage->mip[level].data);
      }
    } else {
      for (int32 level = 0, mipLevels = srcimage->mipLevels; level < mipLevels;
           ++level) {
        glTexImage3D(_res.type, level, srcimage->internalFormat,
                     srcimage->mip[level].width, srcimage->mip[level].height,
                     srcimage->mip[level].depth, 0, srcimage->format,
                     srcimage->type, srcimage->mip[level].data);
      }
    }
...
void TextureRectangle::InitStorage() {
  ...
  if (srcimage != nullptr) {
    glTexImage2D(_res.type, 0, srcimage->internalFormat, srcimage->mip[0].width,
                 srcimage->mip[0].height, 0, srcimage->format, srcimage->type,
                 srcimage->mip[0].data);
...
void TextureCube::SetPositiveX(ImagePtr image) { _imagePositiveX = image; }
void TextureCube::SetPositiveY(ImagePtr image) { _imagePositiveY = image; }
void TextureCube::SetPositiveZ(ImagePtr image) { _imagePositiveZ = image; }

void TextureCube::SetNegativeX(ImagePtr image) { _imageNegativeX = image; }
void TextureCube::SetNegativeY(ImagePtr image) { _imageNegativeY = image; }
void TextureCube::SetNegativeZ(ImagePtr image) { _imageNegativeZ = image; }
...
void TextureCube::InitStorage() {
  ...
  if (_image != nullptr) {
    ...
    useOthers = (_imagePositiveX != nullptr) && (_imagePositiveY != nullptr) &&
                (_imagePositiveZ != nullptr) && (_imageNegativeX != nullptr) &&
                (_imageNegativeY != nullptr) && (_imageNegativeZ != nullptr);
  }
  ...
  // if (srcimage != nullptr)
  {
    ...
    if (_immutable) {
      if (!useOthers && srcimage != nullptr) {
        ...
        glTexStorage2D(_res.type, data->mipLevels, data->internalFormat,
                       data->mip[0].width, data->mip[0].height);

        InitTexStorage(GL_TEXTURE_CUBE_MAP_POSITIVE_X,
                       _imagePositiveX->GetImageSourcePtr());
        InitTexStorage(GL_TEXTURE_CUBE_MAP_NEGATIVE_X,
                       _imageNegativeX->GetImageSourcePtr());
        InitTexStorage(GL_TEXTURE_CUBE_MAP_POSITIVE_Y,
                       _imagePositiveY->GetImageSourcePtr());
        InitTexStorage(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y,
                       _imageNegativeY->GetImageSourcePtr());
        InitTexStorage(GL_TEXTURE_CUBE_MAP_POSITIVE_Z,
                       _imagePositiveZ->GetImageSourcePtr());
        InitTexStorage(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z,
                       _imageNegativeZ->GetImageSourcePtr());
      }
    } else {
      if (!useOthers && srcimage != nullptr) {
        GLubyte *ptr = nullptr;
        for (int32 level = 0, mipLevels = srcimage->mipLevels;
             level < mipLevels; ++level) {
          ptr = (GLubyte *)srcimage->mip[level].data;
          for (int32 face = 0; face < 6; face++) {
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, level,
                         srcimage->internalFormat, srcimage->mip[level].width,
                         srcimage->mip[level].height, 0, srcimage->format,
                         srcimage->type, ptr + srcimage->sliceStride * face);
          }
        }
      } else if (useOthers) {
        ...
        for (int32 face = 0; face < 6; face++) {
          for (int32 level = 0, mipLevels = datas[face]->mipLevels;
               level < mipLevels; ++level) {
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, level,
                         datas[face]->internalFormat,
                         datas[face]->mip[level].width,
                         datas[face]->mip[level].height, 0, datas[face]->format,
                         datas[face]->type, datas[face]->mip[level].data);
          }
        }
      }
    }
...
void TextureCube::InitTexStorage(int32 target, ImageData *data) {
  for (int32 level = 0, mipLevels = data->mipLevels; level < mipLevels;
       ++level) {
    glTexSubImage2D(target, level, 0, 0, data->mip[level].width,
                    data->mip[level].height, data->format, data->type,
                    data->mip[level].data);
  }
}
  ...
  for (int32 level = 0, mipLevels = data->mipLevels; level < mipLevels;
    ...
    for (int face = 0; face < 6; face++) {
      glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, level, 0, 0,
                      data->mip[level].width, data->mip[level].height,
                      data->format, data->type, ptr + data->sliceStride * face);
    }
  }
}
...
void Texture1DArray::InitStorage() {
  ...
  if (srcimage != nullptr) {
    if (_immutable) {
      glTexStorage2D(_res.type, srcimage->mipLevels, srcimage->internalFormat,
                     srcimage->mip[0].width, srcimage->slices);
      for (int level = 0; level < srcimage->mipLevels; ++level) {
        glTexSubImage2D(_res.type, level, 0, 0, srcimage->mip[level].width,
                        srcimage->slices, srcimage->format, srcimage->type,
                        srcimage->mip[level].data);
      }
    } else {
      for (int32 level = 0, mipLevels = srcimage->mipLevels; level < mipLevels;
           ++level) {
        glTexImage2D(_res.type, level, srcimage->internalFormat,
                     srcimage->mip[level].width, srcimage->slices, 0,
                     srcimage->format, srcimage->type,
                     srcimage->mip[level].data);
      }
    }
  ...
  if (srcimage != nullptr) {
    if (_immutable) {
      ...
      for (int32 level = 0, mipLevels = srcimage->mipLevels; level < mipLevels;
           ++level) {
        glTexSubImage3D(_res.type, level, 0, 0, 0, srcimage->mip[level].width,
                        srcimage->mip[level].height, srcimage->slices,
                        srcimage->format, srcimage->type,
                        srcimage->mip[level].data);
      }
    } else {
      for (int32 level = 0, mipLevels = srcimage->mipLevels; level < mipLevels;
        ...
        glTexImage3D(_res.type, level, srcimage->internalFormat,
                     srcimage->mip[level].width, srcimage->mip[level].height,
                     srcimage->slices, 0, srcimage->format, srcimage->type,
                     srcimage->mip[level].data);
      }
    }
...
void Texture2DMultiSampleArray::InitStorage() {
  ...
  glTexStorage3DMultisample(_res.type, _samples, _internalformat, _width,
                            _height, _depth, _fixedsampledlocation);
