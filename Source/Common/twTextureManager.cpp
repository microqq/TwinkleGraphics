...
TextureReader::TextureReader(TextureOption *option) : ResourceReader() {
  if (option != nullptr) {
    if (_option != nullptr) {
      SAFE_DEL(_option);
    }
...
ReadResult<Texture> TextureReader::Read(const char *filename) {
  ...
  case TextureType::TEXTURE_1D:
    texture =
        std::make_shared<Texture1D>(option->_immutable, option->_genMipMap);
    break;
  case TextureType::TEXTURE_1D_ARRAY:
    texture = std::make_shared<Texture1DArray>(option->_immutable,
                                               option->_genMipMap);
    break;
  case TextureType::TEXTURE_2D:
    texture =
        std::make_shared<Texture2D>(option->_immutable, option->_genMipMap);
    break;
  case TextureType::TEXTURE_2D_ARRAY:
    texture = std::make_shared<Texture2DArray>(option->_immutable,
                                               option->_genMipMap);
    break;
  case TextureType::TEXTURE_3D:
    texture =
        std::make_shared<Texture3D>(option->_immutable, option->_genMipMap);
    break;
  case TextureType::TEXTURE_CUBE_MAP:
    texture =
        std::make_shared<TextureCube>(option->_immutable, option->_genMipMap);
    break;
  case TextureType::TEXTURE_CUBE_MAP_ARRAY:
    texture = std::make_shared<TextureCubeArray>(option->_immutable,
                                                 option->_genMipMap);
    break;
  ...
  if (pos != std::string::npos) {
    ...
    if (image != nullptr) {
      ...
      return ReadResult<Texture>(shared_from_this(), texture,
                                 ReadResult<Texture>::Status::SUCCESS);
    }
  }
...
TextureManager &TextureMgrInstance() {
  return Singleton<TextureManager>::Instance();
}

TexturePtr TextureManager::ReadTexture(const char *filename,
                                       TextureOption *option) {
  ResourceManager &resMgr = ResourceMgrInstance();
  std::string texFilename = "Texture:" + std::string(filename);
  ReadResult<Texture> result =
      resMgr.Read<TextureReader, Texture, TextureOption>(texFilename.c_str(),
                                                         option);
...
template <>
void ResourceManager::PackedReadTask<ReadResult<Texture>,
                                     TextureReader>::PushTask() {
  // typedef Ret(R::*)(const char*, ReaderOption) Func;
  ResourceManager &resMgr = ResourceMgrInstance();
  if (_asyncRead) {
    auto future =
        resMgr.PushAsyncTask(&TextureReader::ReadAsync, _reader, _filename);
    {
