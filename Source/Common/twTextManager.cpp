...
TextSource::Ptr TextManager::ReadText(const char *filename) {
  ...
  ReadResult<TextSource> result =
      resMgr.Read<TextReader, TextSource, ReaderOption>(filename, nullptr);
...
ReadResult<TextSource> TextManager::ReadTextAsync(const char *filename) {
  ResourceManager &resMgr = ResourceMgrInstance();
  return resMgr.ReadAsync<TextReader, TextSource, ReaderOption>(filename,
                                                                nullptr);
}
...
template <>
void ResourceManager::PackedReadTask<ReadResult<TextSource>,
                                     TextReader>::PushTask() {
  // typedef Ret(R::*)(const char*, ReaderOption) Func;
  ResourceManager &resMgr = ResourceMgrInstance();
  if (_asyncRead) {
    auto future =
        resMgr.PushAsyncTask(&TextReader::ReadAsync, _reader, _filename);
    {
