...
bool ResourceManager::AddResourceCache(CacheHint hint,
                                       ResourceCache::Ptr cache) {
  std::lock_guard<std::mutex> lock(_cacheMutex);

  if (CacheHint::CACHE_OBJECT == hint) {
    _objectCacheMap.insert(
...
void ResourceManager::UpdateResourceCache(float deltaTime) {
  ...
  for (auto it = _sceneObjectsCacheMap.begin();
       it != _sceneObjectsCacheMap.end();) {
    if (it->second->Expired(deltaTime)) {
...
ResourceCache::Ptr ResourceManager::GetResourceCache(CacheHint hint,
                                                     CacheId id) {
  std::lock_guard<std::mutex> lock(_cacheMutex);
...
void ResourceManager::ClearWorkerPool() { _workerPool.Stop(true); }

void ResourceManager::OnReadTaskSuccess(ObjectPtr obj, ReadTaskId taskid,
                                        CacheHint cachehint,
                                        CacheStoreHint storeHint,
                                        CacheId cacheid, float storeTime,
                                        ReaderId readerid,
                                        ResourceReaderPtr reader) {
  ResourceCache::Ptr cache =
      std::make_shared<ResourceCache>(cacheid, obj, storeHint, storeTime);
  AddResourceCache(cachehint, cache);
  ReleaseTask(obj, taskid, true);
  RecycleReader(readerid, reader);
}

void ResourceManager::OnReadTaskFailed(ReadTaskId taskid, ReaderId readerid,
                                       ResourceReaderPtr reader) {
...
void ResourceManager::ReleaseTask(ObjectPtr obj, ReadTaskId taskid,
                                  bool success) {
  ...
  if (exist) {
    ...
    Console::LogWithColor<Console::Color::GREEN>("Erased tasks (", taskid,
                                                 ") that wait to load count ",
                                                 eraseCount, ".\n");
  }
}
