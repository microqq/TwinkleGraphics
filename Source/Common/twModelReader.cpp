...
ModelReader::ModelReader(ReaderOption *option) : ResourceReader() {
  if (option != nullptr) {
    if (_option != nullptr) {
      SAFE_DEL(_option);
    }
...
ReadResult<Model> ModelReader::Read(const char *filename) {
  ...
  GeometryPtr rootGeom =
      ProcessNode(scene->mRootNode, scene, directory, model, vectorMaterials);
  model->SetRootGeometry(rootGeom);

  SAFE_DEL_ARR(vectorMaterials);

  return ReadResult<Model>(shared_from_this(), model,
                           ReadResult<Model>::Status::SUCCESS);
}
 ...
 */
GeometryPtr ModelReader::ProcessNode(aiNode *node, const aiScene *scene,
                                     std::string dir, ModelPtr model,
                                     MaterialPtr *vecMats) {
  int32 verticeNum = 0;
  ...
  if (verticeNum > 0) {
    ...
    for (unsigned int i = 0; i < node->mNumMeshes; i++) {
      ...
      if (vecMats[mesh->mMaterialIndex] != nullptr) {
        int32 matIndex =
            renderer->GetMaterialIndex(vecMats[mesh->mMaterialIndex]);
        if (matIndex == -1) {
          subMesh->SetMaterialIndex(renderer->GetMaterialCount());
          renderer->AddMaterial(vecMats[mesh->mMaterialIndex]);
          vecMats[mesh->mMaterialIndex] =
              renderer->GetMaterial(renderer->GetMaterialCount() - 1);
        } else {
          subMesh->SetMaterialIndex(
              renderer->GetMaterialIndex(vecMats[mesh->mMaterialIndex]));
        }
      } else {
        MaterialPtr material = ProcessMaterial(
            mesh, scene, scene->mMaterials[mesh->mMaterialIndex], dir,
            (VertexLayoutFlag)meshFlag);
  ...
  for (unsigned int i = 0; i < node->mNumChildren; i++) {
    GeometryPtr childGeom =
        ProcessNode(node->mChildren[i], scene, dir, model, vecMats);
 ...
 *   https://learnopengl.com/
 */
SubMeshPtr ModelReader::ProcessMesh(aiMesh *mesh, MeshPtr tMesh, int32 offset,
                                    const aiScene *scene) {
  ...
  glm::vec4 *uvs[AI_MAX_NUMBER_OF_TEXTURECOORDS] = {
      verticeUV,  verticeUV1, verticeUV2, verticeUV3,
 ...
 *   https://learnopengl.com/
 */
MaterialPtr ModelReader::ProcessMaterial(aiMesh *mesh, const aiScene *scene,
                                         aiMaterial *mat, std::string dir,
                                         VertexLayoutFlag layoutFlag) {
  ...
  // 1. diffuse maps
  std::vector<TexturePtr> diffuseMaps =
      LoadTextures(aiMat, aiTextureType_DIFFUSE, dir);
  SetMaterialTextures(diffuseMaps, material, "diffuseTex", "#define DIFFTEX");

  // 2. specular maps
  std::vector<TexturePtr> specularMaps =
      LoadTextures(aiMat, aiTextureType_SPECULAR, dir);
  SetMaterialTextures(specularMaps, material, "specularTex", "#define DIFFTEX");

  // 3. normal maps
  std::vector<TexturePtr> normalMaps =
      LoadTextures(aiMat, aiTextureType_NORMALS, dir);
  SetMaterialTextures(normalMaps, material, "normalTex", "#define DIFFTEX");

  // 4. height maps
  std::vector<TexturePtr> heightMaps =
      LoadTextures(aiMat, aiTextureType_HEIGHT, dir);
  SetMaterialTextures(heightMaps, material, "heightTex", "#define DIFFTEX");

  // 5. amibient maps
  std::vector<TexturePtr> ambientMaps =
      LoadTextures(aiMat, aiTextureType_AMBIENT, dir);
  SetMaterialTextures(ambientMaps, material, "ambientTex", "#define DIFFTEX");

  // 6. emissive maps
  std::vector<TexturePtr> emissiveMaps =
      LoadTextures(aiMat, aiTextureType_EMISSIVE, dir);
  SetMaterialTextures(emissiveMaps, material, "emissiveTex", "#define DIFFTEX");

  // 7. shininess maps
  std::vector<TexturePtr> shininessMaps =
      LoadTextures(aiMat, aiTextureType_SHININESS, dir);
  SetMaterialTextures(shininessMaps, material, "shininessTex",
                      "#define DIFFTEX");

  // 8. opacity maps
  std::vector<TexturePtr> opacityMaps =
      LoadTextures(aiMat, aiTextureType_OPACITY, dir);
  SetMaterialTextures(opacityMaps, material, "opacityTex", "#define DIFFTEX");

  // 9. displacement maps
  std::vector<TexturePtr> displacementMaps =
      LoadTextures(aiMat, aiTextureType_DISPLACEMENT, dir);
  SetMaterialTextures(displacementMaps, material, "displacementTex",
                      "#define DIFFTEX");

  // 10. lightmap maps
  std::vector<TexturePtr> lightmapMaps =
      LoadTextures(aiMat, aiTextureType_LIGHTMAP, dir);
  SetMaterialTextures(lightmapMaps, material, "lightmapTex", "#define DIFFTEX");

  // 11. reflection maps
  std::vector<TexturePtr> reflectionMaps =
      LoadTextures(aiMat, aiTextureType_REFLECTION, dir);
...
std::vector<TexturePtr> ModelReader::LoadTextures(aiMaterial *mat,
                                                  aiTextureType type,
                                                  std::string dir) {
  ...
  for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
    ...
    // int32 a = 5;
    // if(std::string(str.C_Str()).find("sponza_arch_diff") && type ==
    ...
    ReadSuccessCallbackFuncPtr funcPtr =
        std::make_shared<ReadSuccessCallbackFunc>([texture](ObjectPtr obj) {
          ImagePtr image = std::dynamic_pointer_cast<Image>(obj);
          if (image != nullptr) {
            ImageData *imageData = image->GetImageSourcePtr();
            float fwh = imageData->mip[0].width > imageData->mip[0].height
            ...
            texture->SetFilter<FilterParam::MIN_FILTER>(
            ...
            texture->SetMipMapMaxLevel(MipMapMaxLevelParam::MAX_LEVEL,
                                       level - 1);
