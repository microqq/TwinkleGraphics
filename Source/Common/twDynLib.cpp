...
DynLibManager::~DynLibManager() { _dynlibs.clear(); }
...
DynLib::DynLib(std::string &name)
    : _name(name), _handle(nullptr), _loaded(false), _unloaded(false) {}
...
void DynLib::Load() {
  ...
  nameCorrect = _name.find(".so") != std::string::npos;
#elif __APPLE__
    nameCorrect = _name.find(".dylib") != std::string::npos;
#elif _WIN32
  nameCorrect = _name.substr(_name.find_last_of(".") + 1) == "dll";
#endif
