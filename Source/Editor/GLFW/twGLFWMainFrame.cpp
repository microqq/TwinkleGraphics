...
void GLFWMainFrame::Initialize() {
  ...
  _window = glfwCreateWindow(_pData->width, _pData->height, "Twinkle Editor",
                             NULL, NULL);
  if (!_window) {
    glfwTerminate();
    return;
...
void GLFWMainFrame::BeginFrame() {
  ...
  if (nullptr != viewport && nullptr == dockSpaceLayout->GetImGuiViewport()) {
