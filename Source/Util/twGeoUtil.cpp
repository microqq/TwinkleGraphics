...
NormalizedCubeSphere *CreateNorCubeSphere(float32 radius, int32 subdivision,
                                          MeshDataFlag flag) {
  NormalizedCubeSphere *norcube_sphere =
      new NormalizedCubeSphere(radius, subdivision, flag);
...
IcosahedronSphere *CreateIcosahedronSphere(float32 radius, int32 subdivision,
                                           MeshDataFlag flag) {
  IcosahedronSphere *icosphere =
      new IcosahedronSphere(radius, subdivision, flag);
...
BezierCurve *CreateBezierCurve(glm::vec4 *points, int32 numPoints, int32 degree,
                               int32 segments) {
  BezierCurve *bezierCurve =
      new BezierCurve(points, numPoints, degree, segments);
  // bezierCurve->GenerateCurve();

  return bezierCurve;
}

BSplineCurve *CreateBSplineCurve(glm::vec4 *points, int32 num, Knot *knots,
                                 int32 knots_num, int32 degree,
                                 int32 segments) {
  BSplineCurve *bsplineCurve =
      new BSplineCurve(num, degree, knots_num, knots, points, segments);
...
NURBSSurface *CreateNURBSSurface(glm::vec4 *points, Knot *uknots, Knot *vknots,
                                 int32 unum, int32 vnum, int32 udegree,
                                 int32 vdegree, int32 subdivide, bool rational,
                                 MeshDataFlag flag) {
  NURBSSurface *nurbsSurface =
      new NURBSSurface(unum, udegree, vnum, vdegree, points, uknots, vknots,
...
Plane *CreateInifinitePlane(glm::vec3 normal, float distance, float32 width,
  ...
  Plane *plane =
      new Plane(glm::vec3(0.0f, 0.0f, 1.0f), width, subdivision, flag);
  // plane->GenerateMeshInternal();
